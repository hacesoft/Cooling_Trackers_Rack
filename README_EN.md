...
Simple function for controlling a fan connected via SHELLY PLUG IN based on data from the MPTT tracker VICTRON. The function is implemented in the NODE-RED environment.

Function Description: The current power is read from the MPTT tracker, it is summed, and when the power is equal to or greater than 1 kW, the SHELLY PLUG IN socket is turned on using the command:
 
```
http://xx.xx.xx.xx/relay/0?turn=on 
 ```

Instead of xx.xx.xx.xx, you need to insert the custom IP address of the SHELLY PLUG IN. To do this, edit the SUB_TrackerS node and look for the variable:**sIpAddress**
 

You will change it to the address you need, for example:   
```
const sIpAddress = "192.168.1.10"; 
```
Don't forget to confirm the change and perform a Deploy.

The Java code also includes hysteresis to prevent the fans from turning on and off in 5-second intervals when the current power of Victron solar chargers is around 1kW. In the code, there is a variable nHysteresisTime that specifies the change time for up to 5 minutes (300 seconds).

The fans will turn off when the power falls below 1kW using the command:
```
http://xx.xx.xx.xx/relay/0?turn=off
```

Code node SUB_TrackerS:
```
//*****************************************************************************
//
...
Execution in RED-NODE
![Colling_node_red.png](./Colling_node_red.png)


The code for the entire implementation:
```
